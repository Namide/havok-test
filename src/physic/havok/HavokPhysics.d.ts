...
export interface HavokPhysicsWithBindings extends EmscriptenModule {
  ...
  HP_Shape_AddChild(
    container: HP_ShapeId,
    newChild: HP_ShapeId,
    containerFromChild: QSTransform,
  ): Result;
  ...
  HP_Shape_PathIterator_GetNext(
    curItem: ShapePathIterator,
  ): [Result, ShapePathIterator, number];
  ...
  HP_Constraint_GetAxisMode(
    constraint: HP_ConstraintId,
    axis: ConstraintAxis,
  ): [Result, ConstraintAxisLimitMode];
  ...
  HP_QueryCollector_GetPointProximityResult(
    collector: HP_CollectorId,
    hitIndex: number,
  ): [Result, PointProximityResult];
  /** Get the proximity result stored at hitIndex in the collector.
   * Only valid if the last query performed with this collector was a point proximity. */
  HP_QueryCollector_GetShapeProximityResult(
    collector: HP_CollectorId,
    hitIndex: number,
  ): [Result, ShapeProximityResult];
